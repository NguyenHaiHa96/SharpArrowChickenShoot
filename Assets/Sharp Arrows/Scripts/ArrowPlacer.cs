...
public class ArrowPlacer : MonoBehaviour
{
    ...
    public void BalloonTouchedCallback(Collider[] touchedBalloons, int count) 
    {
        foreach(Collider balloonCollider in touchedBalloons)
        {
            ...
            int arrowsToRemove = (int)Mathf.Pow(Mathf.Sqrt(2), count);
            RemoveArrows(arrowsToRemove, true);
            balloonCollider.GetComponent<Balloon>().Pop();
            LevelManager.Instance.countRemainderBallon--;
            if (LevelManager.Instance.countRemainderBallon <= 0)
            {
                SetLevelComplete();
                return;
            }
    ...
    private void RemoveArrows(int amount, bool poppingBalloons = false)
    {
        for (int i = 0; i < amount; i++)
        {
            ...
            arrowsParent.GetChild(arrowsParent.childCount-1).GetComponent<Arrow>().OnDespawn();
            UpdateArrowCount();
        }
    ...
    {
        Arrow arrow = SimplePool.Spawn<Arrow>(PoolType.Arrow);
        arrow.TF.SetParent(arrowsParent);
        arrow.TF.localPosition = Vector3.zero;
        arrow.TF.localEulerAngles = Vector3.zero;
    }
